...
# Load TTS model for voice cloning (XTTS v2)
...
def clone_voice(audio_file):
    """
    This method will use the XTTS model to clone the voice from the provided audio file.
    """
    cloned_voice_file = "cloned_voice.wav"
    # Clone the voice using Coqui XTTS
    tts_model.tts_to_file(
        text="This is a cloned voice sample",  # Optional: Replace with actual text
        speaker_wav=audio_file,
        language="en",  # You can adjust the language here or make it dynamic
        file_path=cloned_voice_file
    )
    return cloned_voice_file
...
def extract_voice():
    ...
    if file:
        ...
        return jsonify({"message": "Voice extracted and cloned successfully", "voice_file": cloned_voice}), 200
